...
class Panel extends Nette\Object implements Tracy\IBarPanel
{
	...
	public function getPanel()
	{
		...
		$s = "";
		foreach ($this->queries as $query) {
			$s .= "<tr><td>" . sprintf('%0.2f', $query->time * 1000) . "</td>"
				. "<td>" . $query->fname . "(<a href='#tracy-debug-Skautis-args-row-$cnt' class='tracy-toggle tracy-collapsed' rel='#tracy-debug-Skautis-args-row-$cnt'>Args</a>)"
				. "<div id='tracy-debug-Skautis-args-row-$cnt' class='tracy-collapsed'>" . Debugger::dump(reset($query->args[0]), TRUE) . "</div></td>"
				. "<td><a href='#tracy-debug-Skautis-row-$cnt' class='tracy-toggle tracy-collapsed'>Result</a>"
				. "<div id='tracy-debug-Skautis-row-$cnt' class='tracy-collapsed'>" . Debugger::dump($query->result, TRUE) . "</div></td>"
				. "<td>" . $this->prepareTrace($query->trace) . "</td></tr>";
			$cnt++;
		}
